...
class StringComparator:
    ...
    def run(self, quantum_instance=None):
        ...
        result = self.quantum_instance.execute(self.circuit)
        results_raw = result.get_counts(self.circuit)
...
class addition(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        for inp in input_data:        
            circ, key = get_addition_circuit(inp[0], inp[1])
            circuit_list.append(circ)
            
        result = quantum_instance.execute(circuit_list)
        Counts = result.get_counts()

        total_data = {}
        for inp,counts in zip(input_data,Counts):
...
class simon(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        for inp in input_data:
            ...
            circuit_list.append(simon_circuit)
...
class ghz(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        if self.key_aurguments["circuit"]:
            self.key_aurguments["circuit"] = circuit_list
        result = quantum_instance.execute(circuit_list)
        Counts = result.get_counts()
...
class similarity(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        for inp in input_data:
            ...
            comparator = StringComparator(inp[0], inp[1:], symbol_length=2, shots=number_of_runs, quantum_instance=quantum_instance)
            circuit_list.append(comparator)
        
        result = quantum_instance.execute([x.circuit for x in circuit_list])
        raw = result.get_counts()
        
        total_data = {}
        for inp, results_raw,comparator in zip(input_data,raw,circuit_list):
            # tweak raw results and add those strings that have 0 shots/pulses associated with them
            # these are the strings that will have hamming distance equal to the total number of symbols
            for string in comparator.string_db:
                full_binary_string = string[::-1] + "1"
                if full_binary_string not in results_raw:
                    results_raw[full_binary_string] = 0

            # Massage results
            count_dic, useful_shots_count = comparator._get_count_of_useful_values(results_raw)
            p_values = []

            for my_str in comparator.string_db:
                p_values.append(count_dic[my_str] / useful_shots_count)
            ...
            ham_distances = comparator._convert_p_value_to_hamming_distance(p_values, probability_of_measuring_register_c_as_0)
...
class similarity_M1(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        if self.key_aurguments["circuit"]:
            self.key_aurguments["circuit"] = comparator.circuit
