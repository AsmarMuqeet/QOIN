...
temp = os.listdir("evaluation_data/")
#for proc in tqdm(range(0,len(temp),100)):
#    p1 = multiprocessing.Process(target=process, args=(temp[proc:proc+100], ))
#    p1.start()
#    p1.join()
#    time.sleep(60)
...
for remaining in temp:
    bk,name = remaining.split(".")[0].split("_")
    if os.path.exists("results/data/{}_{}.json".format(bk,name)):
        continue
    else:
