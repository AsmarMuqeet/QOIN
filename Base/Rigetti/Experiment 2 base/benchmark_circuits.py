...
class RandomCircuit(abstract_interface):
    ...
    def __init__(self, name="", **kwargs):
        ...
        self.circ = QuantumCircuit.from_qasm_file(
            "random_circuits/{}".format(name))
            
        self.transpiler_bk = FakeLagosV2()
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        if isinstance(input_data, list):
            ...
            for inputs in input_data:
                ...
                circuits.append(transpile(qc, self.transpiler_bk))
            ...
            for inputs, counts in zip(input_data, results):
                ...
                for k, v in counts.items():
                    ...
                    try:
                        odds = v / \
                            sum([value for key, value in counts.items() if key != k])
                        prob = v / \
                    ...
                    data["probability"].append(
                        {"bin": bin_str, "count": v, "odds": odds, "prob": prob})
            ...
            qc.append(sub_inst, self.circ.qubits)
            qc = add_measurements(qc)
            self.circ = transpile(qc, self.transpiler_bk)
    ...
    if selection:
        with open("selected_circuits.txt", "r") as file:
            ...
            for name in tqdm(names):
                name = name.split(".")[0]
                programs[name] = RandomCircuit(
                    name+".qasm", ID=int(name.replace("P", "")), input_type=1)
                mutant = name
                programs_with_mutation[mutant+"_M1"] = RandomCircuit(
                    mutant+"_M1.qasm", ID=int(name.replace("P", "")), input_type=1)
                programs_with_mutation[mutant+"_M2"] = RandomCircuit(
                    mutant+"_M2.qasm", ID=int(name.replace("P", "")), input_type=1)
                programs_with_mutation[mutant+"_M3"] = RandomCircuit(
                    mutant+"_M3.qasm", ID=int(name.replace("P", "")), input_type=1)
    else:
        names = sorted([x for x in os.listdir(
            "./random_circuits/") if "_M" not in x])
        for name in tqdm(names):
            name = name.split(".")[0]
            programs[name] = RandomCircuit(
                name+".qasm", ID=int(name.replace("P", "")), input_type=1)
