...
class StringComparator:
    ...
    def run(self, quantum_instance=None):
        ...
        circ = transpile(self.circuit,basis_gates=["cx", "rz", "sx", "x",'rx', 'ry', 'rz', 'rxx', 'id'])
        results_raw = quantum_instance.execute(circ,shots=self.shots)
...
class cnot(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        counts = backend.execute(circ,shots=number_of_runs)
...
class permutation(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        circ = circ.compose(Permutation(num_qubits = input_data, pattern = generateRandom(input_data)))
...
class expression(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        grover = Grover()
        circ = grover.construct_circuit(problem, 1, measurement=True)
        if self.key_aurguments["circuit"]:
            self.key_aurguments["circuit"] = circ
...
class phase(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        counts = backend.execute(circuit,shots=number_of_runs)
...
class simon(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        counts = backend.execute(simon_circuit,shots=number_of_runs)
...
class similarity(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        comparator = StringComparator(input_A, input_data, symbol_length=2, shots=number_of_runs)
        
        if self.key_aurguments["circuit"]:
            self.key_aurguments["circuit"] = comparator.circuit

        result = comparator.run(backend)
