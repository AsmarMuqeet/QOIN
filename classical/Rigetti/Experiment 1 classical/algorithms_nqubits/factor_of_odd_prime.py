...
class factor_of_odd_prime(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        result = shor.factor(input_data)

        data = {"probability": [], "output": result.factors[0]}
        for k, v in result.counts.items():
            bin_str = k
            dec_str = convert_to_int(k)
            str_str = convert_to_str(k)
            try:
                odds = v / sum([value for key, value in result.counts.items() if key != k])
                prob = v / sum([value for key, value in result.counts.items()])
            except:
                odds = 1
    ...
    def get_happy_scene(self):
        return {"input": 15, "output": [3, 5], "probability": [
            {'bin': '10000000', 'dec': 128, 'ascii': '\x80', 'count': 236, 'odds': 0.29949238578680204,
             'prob': 0.23046875},
            {'bin': '11000000', 'dec': 192, 'ascii': 'À', 'count': 266, 'odds': 0.35092348284960423,
             'prob': 0.259765625},
            {'bin': '00000000', 'dec': 0, 'ascii': '\x00', 'count': 291, 'odds': 0.3969986357435198,
             'prob': 0.2841796875},
            {'bin': '01000000', 'dec': 64, 'ascii': '@', 'count': 231, 'odds': 0.29129886506935687,
             'prob': 0.2255859375}
            ]}
